    ...
    return {
        ...
        handleHotUpdate: ctx => {
            for (const file of ctx.modules) {
                ...
                console.log(chalk.yellow(consoleViteMessagePrefix), chalk.yellow("hmr update"), chalk.grey(path));
            }
            return ctx.modules;
        }
    };
...
export default async function startViteServer(): Promise<() => Promise<void>> {
    ...
    if (typeof address === "object") {
        ...
        console.log(chalk.green(consoleViteMessagePrefix), chalk.green(`Dev server running at: localhost:${port}`));
    }
    return async () => {
        await server.close();
    };
