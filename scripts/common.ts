...
function formatCompileError(error: CompileError): string {
    ...
    const code =
        ...
        repeatString(" ", error.location.lineText.length - error.location.column - error.location.length);
...
export function formatDiagnosticsMessage(errors: CompileError[]): string {
    ...
    const res =
        chalk.rgb(255, 161, 237)(`${consoleMessagePrefix} Some typescript compilation errors occurred:`) +
        "\n" +
        diagnosticDetail +
        "\n" +
